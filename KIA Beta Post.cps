...
description = "KIA Beta Post";
vendor = "P3D Creations";
vendorUrl = "https://p3dcreations.com";
legal = "Copyright P3D Creations LLC, Derived from Autodesk Doosan Mill-Turn Post";
certificationLevel = 2;
minimumRevision = 40783;

longDescription = "KIA SKT21LMS Post customized from Doosan Mill-Turn";
...
properties = {
  ...
  machineType: "LYNX", // type of machine "PUMA", "LYNX", "LYNX_YAXIS", "PUMA_MX"
  useG400: false, // use G400 for milling tools
  reverseAxes: true, // reverse YC-axes on sub-spindle
  useSpindlePcodes: false // use P11, P12, P13, etc. to specify the spindle, otherwise use unique M-codes
};
...
function onSection() {
  ...
  if (!isFirstSection() && insertToolCall) {
    if (machineState.spindlesAreSynchronized) {
      if (!machineState.spindlesAreAttached) {
        ...
        writeBlock(mFormat.format(115));
        writeBlock(mFormat.format(161));
  ...
  if (tempSpindle == SPINDLE_LIVE) {
    if (insertToolCall || wcsOut || feedMode) {
      forceUnlockMultiAxis();
      onCommand(COMMAND_UNLOCK_MULTI_AXIS);
      var plane = getMachiningDirection(currentSection) == MACHINING_DIRECTION_AXIAL ? 18 : 18; //Modified to remove G17 from a non-Y axis lathe
	  //var plane = getMachiningDirection(currentSection) == MACHINING_DIRECTION_AXIAL ? getG17Code() : 18; //Original code from the doosan post -NP
      gPlaneModal.reset();
...
function moveSubSpindle(_method, _position, _feed, _useMachineFrame, _comment, _error) {
  ...
  case HOME:
    if ((properties.useTailStock == "false") || !machineState.tailstockIsActive) { // don't retract B-axis if used as a tailstock
      writeBlock(
        gMotionModal.format(0),
        //gFormat.format(28), //Removing G28 because of address duplication error
        gFormat.format(53),
        subOutput.format(0),
...
function ejectPart() {
  ...
  writeBlock(
    ...
    gPlaneModal.format(17), //(Comment this if you have problems- Disable G17 select for C axis mode- No Y axis so it shouldnt be necessary and causes an error)
...
function onClose() {
  ...
  writeBlock(gFormat.format(54));// , mFormat.format(80)); Removed M80, not sure what it's purpose is. 
